...
Client.prototype.order = function() {
    ...
    socket.on('report', function(data){
        ...
        if(action === 'addLabor'){
            ...
            logger.debug('add labor "' + laborId + ' [' + data.info.platform + ']"')
        }else if(action === 'delLabor'){
            logger.debug('delete labor "' + laborId + '"')
...
Client.prototype.printResults = function(){
    ...
    _.each(labors, function(labor, key, labors){
        ...
        var resultInfo = labor.result.info
        var reports =  labor.reports
        var color = resultInfo.failures ? 'red' : 'green'
