...
function LaborManager(){
    ...
    setInterval(function(){
        if(self.waitingTasks.length){
            self.handleFirstWaitingTask()
        }
    }, 50)
}
...
LaborManager.prototype.handleFirstWaitingTask = function(){
    var self = this
...
function isMatch(platform, ua){
    ...
    _.each(ua, function(v, k, ua){
         ...
         */
        if(platform[k] && !v.match(platform[k])){
            matched = false
        }
    })
    return matched
...
function formatPlatform(platform){
    var arr = platform.toLowerCase().split('/')
    var len = arr.length
    var obj
    // 'os/name/version'
    if(len === 3){
...
function formatUa(ua){
    return {
        os: ua.os.family.toLowerCase(),
        name: ua.family.toLowerCase(),
