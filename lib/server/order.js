...
function Order(id, socket){
    ...
    logger.debug('new order: ' + self.id)
}
...
Order.prototype.delLabor = function(laborId){
    var self = this
    ...
    logger.debug('order: ' + self.id + ' delete not finished labor: ' + laborId)
    self.checkIfEndAll()
}
...
Order.prototype.destroy = function(){
    ...
    _.each(self.labors, function(labor, key, labors){
        labor.instance.delOrder(self.id)
    })
