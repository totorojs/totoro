 ...
 * 目前使用 mocha 的话，则固定是 mocha + expect.js + sinon 的搭配，以后再考虑扩展支持
 * 
 * note：为什么我们不选择以下库？
...
function detectFrames(testFiles) { 
    ...
    for(var i = 0, len = testFiles.length; i < len; i++){
        ...
        if(testSuit === JASMINE){
            rt = {'testFrames': {'jasmine': true}, 'testStyle': BDD}
            break
        }
        ...
        if((testSuit === MOCHA) && testStyle){
            rt = {
                'testFrames': {'mocha': true, 'expect.js': true, 'sinon': true},
                'testStyle': testStyle
            }
            break
        }
