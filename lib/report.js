...
module.exports = function(data) {
  ...
  Object.keys(data).sort().reverse().forEach(function(traitStr) {
    ...
    if (!labor.info || !labor.info.stats || labor.info.stats.failures) {
      rankedData.push([traitStr, labor, 0])
      failedAmount ++
    } else {
      rankedData.unshift([traitStr, labor, 1])
    }
  })

  rankedData.forEach(function(item) {
    println()
    var traitStr = perfectTraitStr(item[0])
    var labor = item[1]
    var tryTimes = labor.tryTimes

    if (tryTimes) {
      println('  ' + traitStr, item[2] ? 'green' : 'red')
      var info = labor.info
...
function perfectTraitStr(traitStr) {
  var str = traitStr.replace(/[ \/]+$/, '')
