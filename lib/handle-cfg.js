...
var env = process.env
var tempDir = path.normalize((env.TMPDIR || env.TMP || env.TEMP || '/tmp') + '/totoro-code')
...
function handleCode(cfg) {
    ...
    scripts.push('<script>totoro.end()</script>')
    ...
    cfg.runner = path.join(tempTestDir, 'runner.html')
    cfg.repo = 'totoro-code-runner'
    cfg.adapter = 'code'
}
