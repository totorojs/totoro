...
function findRunnerRoot(runner) {
    ...
    var SCRIPT_RE = /<script\s+[^>]*src=(["'])(.+?)\1/g
    var LINK_RE = /<link\s+[^>]*href=(["'])(.+?)\1/g
    var stag = html.match(SCRIPT_RE) || []
    var ltag = html.match(LINK_RE) || []
    var tags = stag.concat(ltag)

    var urls = []
    var ABSOLUTE_RE = /^\/\/.|:\//
    stag.forEach(function(tag) {
        var url = SCRIPT_RE.exec(tag)[2]
        if(url.charAt(0) != '/' && !ABSOLUTE_RE.test(url))
            urls.push(url)
    })
    ltag.forEach(function(tag) {
        var url = LINK_RE.exec(tag)[2]
        if(url.charAt(0) != '/' && !ABSOLUTE_RE.test(url))
            urls.push(url)
    })
    urls.map(function(url) {
        return path.dirname(path.normalize(path.join(runner, url)))
    })
