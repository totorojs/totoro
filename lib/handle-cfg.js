...
function findAdapter(cfg) {
    ...
    if (isExistedFile(runner)) {
        autoAdapter = path.join(path.dirname(runner), 'totoro-adapter.js')
        if (isExistedFile(autoAdapter)) {
            cfg.adapter = autoAdapter
            logger.debug('guess adapter is: ' + autoAdapter)
        }
    }
}
