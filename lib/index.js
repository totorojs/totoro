...
function Client(cfg) {
  ...
  this.report = report
  if (cfg.report) {
    if (utilx.isFunction(cfg.report)) {
      this.report = cfg.report
    } else if (utilx.isExistedFile(cfg.report)) {
      this.report = require(cfg.report)
    } else {
      logger.warn('Option report must be a js file path or a function.')
    }
    ;delete cfg.report
  }
...
Client.prototype.launchTest = function() {
  ...
  socket.on('proxyReq', function(info) {
    var parsedRunner = url.parse(cfg.runner)
    var opts = {
