...
function createRunner(testDir) {
    ...
    var testFrames = findTestFrames(testFiles)
    ...
    var deps = findTestDependencies(testFiles, pkg.dependencies || {})
...
// 查找使用的测试框架
function findTestFrames(testFiles) {
...
// 查找所有测试用例的依赖
function findTestDependencies(testFiles, deps) {
    checkJquery(deps)
    return JSON.stringify(_.keys(deps).concat(testFiles))
}
